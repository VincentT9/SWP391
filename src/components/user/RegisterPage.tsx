...
const API_REGISTER_URL = process.env.REACT_APP_REGISTER_API || 'http://localhost:5112/api/Auth/register';
...
const RegisterPage = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Success - backend returns text "Register!!"
      await response.text();
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={{ width: '100%' }}>
          ...
          <Controller
            ...
            rules={{
              ...
              pattern: { value: /^[a-zA-Z0-9_]+$/, message: 'Tên đăng nhập chỉ chứa chữ cái, số và dấu gạch dưới' },
            }}
            render={({ field }) => (
              <TextField
                {...field}
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="body2" color="white">
              Đã có tài khoản?{' '}
              <Link component={RouterLink} to="/login" sx={{ color: 'yellow', textDecoration: 'none' }}>
                Đăng nhập
              </Link>
            </Typography>
          </Box>
