    ...
    <Container maxWidth="lg">
      ...
      {healthRecord ? (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body1" color="text.secondary">
                    Lớp {student.class} • {student.dateOfBirth.toLocaleDateString('vi-VN')}
                  </Typography>
                </Box>
              </Box>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                      {(healthRecord.weight / Math.pow(healthRecord.height / 100, 2)).toFixed(1)}
                    </Typography>
                  </Box>
                </Box>
              </Box>
          ...
          {/* Allergies and Chronic Conditions Container */}
          <Box sx={{ 
            display: 'flex', 
                ...
                {healthRecord.allergies.length > 0 ? (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {healthRecord.allergies.map((allergy) => (
                      <Typography key={allergy.id} variant="body2" sx={{ p: 1, bgcolor: '#fff3e0', borderRadius: 1 }}>
                        <strong>{allergy.name}</strong> ({getSeverityText(allergy.severity)}) - {allergy.symptoms}
                      </Typography>
                    ))}
                  </Box>
                ) : (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <WarningIcon sx={{ fontSize: 48, color: '#bdbdbd', mb: 2 }} />
                    <Typography variant="body1" color="text.secondary">
                ...
                {healthRecord.chronicConditions.length > 0 ? (
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {healthRecord.chronicConditions.map((condition) => (
                      <Typography key={condition.id} variant="body2" sx={{ p: 1, bgcolor: '#ffebee', borderRadius: 1 }}>
                        <strong>{condition.name}</strong> - {condition.notes} (từ {condition.diagnosisDate.toLocaleDateString('vi-VN')})
                      </Typography>
                    ))}
                  </Box>
                ) : (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
                    <HospitalIcon sx={{ fontSize: 48, color: '#bdbdbd', mb: 2 }} />
                    <Typography variant="body1" color="text.secondary">
