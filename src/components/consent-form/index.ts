...
export const getConsentFormsByStudentId = async (studentId: string): Promise<ConsentForm[]> => {
  const res = await instance.get(`/api/ConsentForm/get-consent-forms-by-student-id/${studentId}`);
  return res.data;
};

// Thêm hàm mới để lấy học sinh theo parentId
export const getStudentsByParentId = async (parentId: string): Promise<any[]> => {
  const res = await instance.get(`/api/Student/get-student-by-parent-id/${parentId}`);
  return res.data;
};

// Thêm hàm mới để lấy consent form của phụ huynh (tất cả con)
export const getConsentFormsByParentId = async (parentId: string): Promise<ConsentForm[]> => {
  try {
...
export async function deleteConsentForm(consentFormId: string) {
  const response = await fetch(`/api/ConsentForm/delete-consent-form/${consentFormId}`, {
    method: "DELETE",
  });
  if (!response.ok) {
...
export const getConsentFormsByCampaign = async (campaignId: string): Promise<ConsentForm[]> => {
  const res = await instance.get(`/api/ConsentForm/get-consent-forms-by-campaign-id/${campaignId}`);
  return res.data;
};
...
export async function updateConsentForm(consentFormId: string, payload: {
  campaignId: string;
  studentId: string;
  isApproved: boolean;
  consentDate: string;
  ...
  await instance.put(`/api/ConsentForm/update-consent-form/${consentFormId}`,  payload );
};