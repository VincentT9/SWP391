...
import { getConsentFormsByStudentId, getConsentFormsByCampaign, getConsentFormsByParentId, updateConsentForm, getAllConsentForms, deleteConsentForm } from "./index";
...
const ConsentFormPage: React.FC<ConsentFormPageProps> = ({ mode, studentId, parentId, campaignId, scheduleId }) => {
  ...
  const getStatusColor = (isApproved?: boolean, updatedBy?: string | null): "default" | "success" | "error" | "warning" => {
  ...
  const getStats = () => {
    ...
    const approved = consentForms.filter(f => f.updatedBy && f.isApproved === true).length;
    const rejected = consentForms.filter(f => f.updatedBy && f.isApproved === false).length;
  ...
  const filteredForms = consentForms.filter(form => {
    ...
    if (statusFilter === "approved") return form.updatedBy && form.isApproved === true;
    if (statusFilter === "rejected") return form.updatedBy && form.isApproved === false;
        ...
        <Box sx={{ mb: 4 }}>
          <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 2 }}>
            ...
            <Box>
              ...
              >
                {mode === "parent" ? "Phiếu Đồng Ý Tiêm Chủng" : "Quản Lý Phiếu Đồng Ý"}
              </Typography>
          ...
          >
              ...
              <CardContent sx={{ p: 3, position: 'relative', zIndex: 1 }}>
                ...
                <Box sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
                  <Typography variant="caption">
                    {stats.total > 0 ? Math.round((stats.approved / stats.total) * 100) : 0}% tỷ lệ chấp nhận
                  </Typography>
                </Box>
              </CardContent>
            </Card>
            ...
            <Card sx={{ 
              ...
              <CardContent sx={{ p: 3, position: 'relative', zIndex: 1 }}>
                ...
                <Box sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
                  <Typography variant="caption">
                    {stats.total > 0 ? Math.round((stats.rejected / stats.total) * 100) : 0}% tỷ lệ từ chối
                  </Typography>
                </Box>
              </CardContent>
            </Card>
                      ...
                      <React.Fragment key={form.id}>
                        ...
                        >
                          <Stack direction="row" spacing={3} alignItems="center" width="100%">
                            <Avatar sx={{ 
                              bgcolor: getStatusColor(form.isApproved, form.updatedBy) === 'success' ? 'success.main' :
                                      getStatusColor(form.isApproved, form.updatedBy) === 'error' ? 'error.main' :
                                      getStatusColor(form.isApproved, form.updatedBy) === 'warning' ? 'warning.main' : 'primary.main',
                              width: 48,
                              height: 48,
                              boxShadow: 2
                            }}>
                            ...
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              ...
                              <Chip
                                icon={getStatusIcon(form.isApproved, form.updatedBy)}
                                label={getStatusText(form.isApproved, form.updatedBy)}
                                color={getStatusColor(form.isApproved, form.updatedBy)}
                                size="small"
                                variant="filled"
